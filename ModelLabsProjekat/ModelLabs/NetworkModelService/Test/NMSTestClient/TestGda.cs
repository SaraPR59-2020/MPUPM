...
namespace TelventDMS.Services.NetworkModelService.TestClient.Tests
{
	...
	{			
        ...
        public void ReadAllCurvs()
        {
            XmlTextWriter writer = null;
            int iteratorId = 0;
            int resNum = 2;
            int resourcesLeft = 0;
            ModelCode powerTransformerType = ModelCode.CURVEDATA;

            try
            {
                List<ModelCode> ids = modelResourcesDesc.GetAllPropertyIds(powerTransformerType);

                iteratorId = GdaQueryProxy.GetExtentValues(powerTransformerType, ids);
                resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);

                writer = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\AllCurveDatas.xml", Encoding.Unicode);
                ...
                Console.WriteLine("Reading successfully finished.");
            }
            catch (Exception e)
            {
        ...
        private void ReadResourcesAndWriteToXML(XmlTextWriter writer, int iteratorId, int resNum, int resourcesLeft)
        {
            while (resourcesLeft > 0)
            {
                List<ResourceDescription> rds = GdaQueryProxy.IteratorNext(resNum, iteratorId);
        ...
        public void ReadCurvDataLowestGID()
        {
            XmlTextWriter writer = null;
            int iteratorId = 0;
            List<long> ids = new List<long>();
            int resNum = 2;
            int resourcesLeft = 0;
            ModelCode powertrModelCode = ModelCode.CURVE;

            try
            {
                List<ModelCode> props = modelResourcesDesc.GetAllPropertyIds(powertrModelCode);

                iteratorId = GdaQueryProxy.GetExtentValues(powertrModelCode, props);
                resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);
            ...
            association.PropertyId = ModelCode.CURVE_CURVEDATAS;
            association.Type = ModelCode.CURVEDATA;
            ...
            try
            {
                ...
                iteratorId = GdaQueryProxy.GetRelatedValues(gid, properties, association);
                resourcesLeft = GdaQueryProxy.IteratorResourcesLeft(iteratorId);

                writer = new XmlTextWriter(Config.Instance.ResultDirecotry + "\\CurvData.xml", Encoding.Unicode);
                writer.Formatting = Formatting.Indented;
